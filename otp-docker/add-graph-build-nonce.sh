...
# Download files
jq -r ".baseFolderDownloads[] | .uri" /var/opentripplanner/otp-runner-graph-build-manifest.json | while read uri; do aws s3 cp $uri /var/opentripplanner/; done
jq -r ".baseFolderDownloads[] | .uri" /var/opentripplanner/otp-runner-graph-build-manifest.json | while read uri; do curl $uri -O /var/opentripplanner/; done